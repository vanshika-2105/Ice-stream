...
# Day 11 – Historical Quality Metrics

## Objective

Extend the Ice-Stream quality monitoring pipeline to persist historical data-quality metrics in Apache Iceberg so that downstream dashboards can analyze quality trends over time.

## Historical Metrics Architecture

The Day 11 pipeline extends the existing streaming architecture:
...
checkout_events
    |
    v
One-Minute Quality Windows
    |
    v
Iceberg quality_metrics
```

Valid checkout events and quality metrics are stored separately so that event-level data and aggregated monitoring data remain independent.

## Metric Windows

Quality metrics are calculated using one-minute tumbling windows based on Flink processing time.

Each completed window produces one historical metrics record.
...
The one-minute window allows the dashboard to monitor changes in data quality continuously over time.

Each window is appended to the `quality_metrics` Iceberg table. Existing metric windows are not overwritten.

## Quality Metrics

For every completed window, Flink calculates:
...
Historical quality metrics are stored in:
...
Each completed metric window creates a new record in the Iceberg table.

This provides historical storage for dashboard trend analysis.

## Historical Metrics Verification

Multiple metric windows were generated by running the checkout event producer.

The historical query returned multiple windows, including:

```text
13:20–13:21  Total: 30   Valid: 28   Invalid: 2
13:21–13:22  Total: 14   Valid: 13   Invalid: 1
13:22–13:23  Total: 59   Valid: 56   Invalid: 3
13:23–13:24  Total: 38   Valid: 32   Invalid: 6
13:29–13:30  Total: 141  Valid: 129  Invalid: 12
13:30–13:31  Total: 1    Valid: 1    Invalid: 0
```

The presence of multiple records confirms that metric windows are retained historically rather than overwritten.

## Latest Quality Windows

The latest quality windows can be retrieved from Iceberg using:
...
SELECT
    ...
    quality_score,
    invalid_event_rate
FROM iceberg_catalog.checkout.quality_metrics
ORDER BY window_start DESC
LIMIT 5;
```

This query allows dashboard components to retrieve the most recent quality measurements.

## Flink Restart and Historical Data

The Flink job was restarted during testing.

After the restart, previously generated quality metric windows remained available in the Iceberg table.

New metric windows were appended after processing resumed.

This confirms that historical quality metrics are persisted independently of the running Flink process.

## Valid Event Storage

Valid events continue to be stored separately in:

```text
iceberg_catalog.checkout.checkout_events
```

The valid-event path was verified by querying the Iceberg table.

The verification returned:

```text
valid_events_stored: 293
```

This confirms that valid checkout events continue to flow through:
...
VALID
  ↓
Iceberg checkout_events
```

## Invalid Event and DLQ Verification

Invalid events continue to be routed to:

```text
checkout-events-dlq
```

The DLQ was verified using the Kafka console consumer.

Invalid records containing validation failures such as:
...
were observed in the DLQ.

This confirms that the invalid-event path remains independent from the historical metrics storage:
...
| Test                                        | Result |
| ------------------------------------------- | ------ |
| One-minute metric windows                   | PASS   |
| Historical metric storage                   | PASS   |
| Multiple metric windows retained            | PASS   |
| Metric append behavior                      | PASS   |
...
Expected quality decreases progr
...
# Day 16 – Unified Pipeline Health Verification
...
Verified Flink job is running and consuming Kafka events.

## Throughput Verification

* Total Events: 100
* Throughput: 35.59 EPS

## DLQ Verification

* Invalid Events: 25
* DLQ Rate: 25%
* Invalid events successfully routed to checkout-events-dlq.

## Failure Test

* Stopped Kafka container.
* Verified component outage.
* Restarted Kafka container.
* Verified producer successfully resumed sending events.
